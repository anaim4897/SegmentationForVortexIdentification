...
velocity_data = torch.load('velocity_magnitude.pt')
print(velocity_data.dtype)
velocity_data = torch.tensor(velocity_data).unsqueeze(1)
vorticity_mask = torch.load('vorticity_mask.pt')
...
# Define your dataset
dataset = VortexDataset(velocity_data, vorticity_mask)
...
for epoch in range(num_epochs):
    ...
    for batch in dataloader:
        ...
        # outputs = nchw_to_nlc(outputs[2])
        conv = nn.Conv2d(in_channels=2, out_channels=1, kernel_size=3)
        outputs = conv(outputs[2])
        print(outputs[2].shape, labels.shape)
        # Calculate loss
        ...
        plt.imshow(labels.squeeze(1)[0].cpu())
        plt.colorbar()
        plt.show()

        plt.title(f"pred {epoch + 1}")
        plt.imshow(outputs.detach().cpu().numpy()[3])
        plt.colorbar()
        plt.show()

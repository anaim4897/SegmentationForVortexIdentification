...
vorticity_mask = torch.load('vorticity_mask.pt').to(device)
...
# Define your model
model = MixVisionTransformer().to(device)
...
for epoch in range(num_epochs):
    ...
    im2 = ax2.imshow(outputs.detach().cpu().numpy()[2])
    plt.colorbar(im2, ax=ax2)

    print(f"label {epoch + 1}: ({torch.where(velocity_data == labels[0])[0]})")
    print(f"pred {epoch + 1}: ({torch.where(velocity_data == outputs[0])[0]})")

# Save the entire figure containing all subplots
fig.savefig("Seg_training_plots_combined.png")  # Save the figure in the current working directory
